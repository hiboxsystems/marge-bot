...
class MergeRequest(gitlab.Resource):
    ...
    def fetch_assigned_at(cls, user, api, merge_request):
        ...
        all_discussions = api.collect_all_pages(
            ...
            GET(f"/projects/{merge_request.get('project_id')}/merge_requests/{merge_request.get('iid')}/discussions")
        )
    ...
    def fetch_all_open_for_user(cls, project_id, user, api, merge_order):
        ...
        my_merge_request_infos = [
            ...
            or (user.id in [assignee.get('id') for assignee in (mri.get('assignees', []) or [])])
        ]
    ...
    def work_in_progress(self):
        return self.info['work_in_progress'] or ('draft' in self.info and self.info['draft'])
