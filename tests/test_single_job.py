...
class SingleJobMockLab(MockLab):
        ...
        *,
        ...
        if expect_gitlab_rebase:
            ...
            api.add_pipelines(
                ...
                from_state='initial', to_state='pipeline-running',
            )
            api.add_transition(
                PUT(
        ...
        api.add_transition(
            GET(
                ...
                f"/projects/{source_project_id}/repository/branches/{self.merge_request_info['source_branch']}",
            ),
        ...
        api.add_transition(
            GET(f"/projects/1234/repository/branches/{self.merge_request_info['target_branch']}"),
    ...
    def expected_failure(self, message):
        ...
        error_note = f"I couldn't merge this merge request: {message}"
...
class TestUpdateAndAccept:  # pylint: disable=too-many-public-methods
    ...
    def options_factory(self, fusion, add_tested, add_reviewers, add_part_of):
        if fusion is Fusion.gitlab_rebase and (add_tested or add_reviewers or add_part_of):
    ...
    def test_succeeds_first_time(self, mocks):
        ...
        if job.opts.fusion is Fusion.gitlab_rebase:
            api.add_pipelines(
                ...
                from_state='pipeline-running', to_state='pipeline-finished',
            )
    ...
    def test_succeeds_with_updated_branch(self, mocks):
        ...
        api.add_transition(
            GET(
                f"/projects/1234/repository/branches/{mocklab.merge_request_info['source_branch']}",
            ),
    ...
    def test_fails_on_not_acceptable_if_master_did_not_move(self, mocks):
        ...
        api.add_transition(
            GET(
                # pylint: disable=line-too-long
                f"/projects/{mocklab.merge_request_info['source_project_id']}/repository/branches/useless_new_feature",
            ),
    ...
    def test_second_time_if_master_moved(self, mocks_factory, fusion, update_sha, rewrite_sha):
        ...
        if fusion is Fusion.gitlab_rebase:
            # pylint: disable=line-too-long
            rebase_url = f"/projects/{mocklab.merge_request_info['project_id']}/merge_requests/{mocklab.merge_request_info['iid']}/rebase"
        ...
        if job.opts.fusion is Fusion.gitlab_rebase:
            ...
            api.add_pipelines(
                ...
                _pipeline(sha1=expected_sha1,
                          status='success',
                          ref=mocklab.merge_request_info['source_branch']),
                from_state='pipeline-running', to_state='passed',
            )
        else:
            expected_sha1 = mocklab.rewritten_sha
    ...
    def test_wont_merge_wip_stuff(self, mocks):
        ...
        with mocklab.expected_failure("Sorry, I can't merge requests marked as Draft!"):
            job.execute()
    ...
    def test_wont_merge_branches_with_autosquash_if_rewriting(self, mocks):
        ...
        if job.opts.fusion is Fusion.gitlab_rebase:
            pytest.skip('Not applicable if GitLab API rebase/merge is used')
    ...
    def test_fails_if_changes_already_exist(self, mocks):
        ...
        assert api.state == 'initial'
        assert api.notes == [f"I couldn't merge this merge request: {expected_message}"]
